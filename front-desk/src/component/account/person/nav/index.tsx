...
export default withRouter(
  class AccountPersonNav extends React.Component<Props, State> {
    public state: State = {
      personNavList: [
        { key: '1', icon: 'heart', name: '我的收藏', path: '/account/person/collection-favorite' },
        { key: '2', icon: 'user', name: '我的信息', path: '/account/person/info' }
      ]
    };
            ...
            <section className="person-nav-list-container">
              {state.personNavList.map((item, index) => (
                <Link href={item.path} key={index}>
                  <a href={item.path} className={this.getPersonNavSelectKey() === item.key ? 'active' : ''}>
                    <span>{item.name}</span>
                  </a>
                </Link>
              ))}
